...
int main(const int argc, const char **argv){
	...
	const int64_t threads = std::stoll(argv[2]);
	...
	if (size <= 0){
		...
		std::cerr << "Error: need at least two threads for this to actually mean anything." << std::endl;
		exit(EXIT_FAILURE);
	...
	std::cout << "Multiplying with OpenMP..." << std::endl;

	const double multiplyStart = getRealTime();

	multiplyOMP(mat1, mat2, mat3, threads);

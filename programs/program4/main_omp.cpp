...
inline void usage(const char *name){
	...
	cout << "Usage: " << name << " <problem_size> <generations> <threads>" << endl;
...
int main(const int argc, const char **argv){
	...
	const int64_t threads = stoll(argv[3]);
	...
	if (gridSize < 3 || numElements < 0){
		...
		cerr << "Thread count " << threads << " is too low." << endl;
		return 1;

	}

	cout << "Building start grid..." << endl;
	cout << "Size: " << gridSize << ", threads: " << threads << endl;

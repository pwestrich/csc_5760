...
void multiplyParallel(Matrix *const L, Matrix *const R, Matrix *const S, const int64_t numThreads){
	...
	dispatch_queue_t *queue = dispatch_create_queue("matrix_queue", QUEUE_CONCORRENT, numThreads);
	...
	for (int64_t i = 0; i < numThreads; ++i){
		...
		if (i == (numThreads - 1)){
			...
			dispatch_sync(queue, multiplyThread, args);	//wait on the last one

		} else {

			dispatch_async(queue, multiplyThread, args);
